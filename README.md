...
measure what the detector can actually see, and try to infer what particle produced it.

## Detector geometry

| Component | Description |
|---|---|
| Silicon trackers | Two cylindrical layers (R = 4, R = 8) inside a 0.5 T solenoid; charged-particle momentum from track sagitta |
...
3. **Particle Type Classification**
   - Rule-based, event-level classifier using only detector observables (no truth
     leakage), with charge then attached per-track.
   - Three calibration schemes compared: a single global factor, an idealized
     per-`TrueType` factor, and a realistic per-`Particle_Type` (i.e. as-classified)
     factor.
   - Energy-deposition and shower-shape profiles per particle species across both
     calorimeters.
4. **Gradient Boosting Classifier (ML)**
   - `HistGradientBoostingClassifier` trained on track- and calorimeter-level features,
     compared directly against the rule-based result on the same held-out events.
   - Permutation feature importance, training/validation accuracy, and 5-fold
     cross-validation to confirm the result isn't a train/test split artifact.
   - A dedicated charge-identification comparison, including an ablation that withholds
     the curvature-sign feature and a binomial significance test against chance.

## Headline results

| Classifier | Overall efficiency | Notes |
|---|---|---|
...
The notebook expects Geant4 ntuple CSV output in a `build/` directory, relative to the
notebook:

```
build/output_nt_EMCal.csv
build/output_nt_HCal.csv
build/output_nt_MuonDetector.csv
build/output_nt_Tracker1.csv
build/output_nt_Tracker2.csv
build/output_nt_Truth.csv
```

These are generated by running the companion Geant4 simulation (see the main project
repo for the simulation source and the `geant4run` build/run helper). Tracker columns
include per-track `EnergyDeposit` and `PathLength`, which support dE/dx-based features
...
- Hadron and photon misclassification still dominate the rule-based confusion matrix;
  further investigation into shower-shape discriminators is ongoing.
- No calorimeter endcaps are currently modelled, which is the primary source of angular
  acceptance loss described above.

## Acknowledgements

Built on Geant4 example code from course workshops, modified for this project's
detector geometry, generator, and sensitive-detector logic (see simulation source for
inline attribution). Project specification: *PDAML Geant4 Project*, Ben Wynne, University
of Edinburgh.